...
export default {
  ...
  created() {
    ...
    $('body').on('mouseup', function(event) {
      _self.handleDragUp(event);
    });
    $('body').on('mousemove', function(event) {
      _self.handleDragMove(event);
    });
  ...
  methods: {
    ...
    handleDown(event, row, index, column) {
      if (column.type === 'index' && event.which === 1) {
        this.dragData = row;
        this.dragIndex = index;
        this.dragItem = $($($(event.currentTarget).parent()));
        this.isDrag = true;
        var _top = this.dragItem.offset().top - $(this.dragItem.parent()).offset().top;
    ...
    handleDragMove(event) {
      ...
      if (this.isDrag) {
        this.preventDefault(event);
        var _top = parseInt($('.elx-drag-item').css('top'), 10) + this.mousePosGap.y;
        var _trTop = this.dragItem.offset().top - $(this.dragItem.parent()).offset().top;
        if (_top < _trTop - this.dragItem.height() / 2) {
          ...
          this.dragItem = $($(this.dragItem.parent()).find('tr')[this.dragIndex]);
