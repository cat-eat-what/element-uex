<template>
  <div class="date-button-group">
    <el-button-group> 
      <el-button v-for = "(type, index) in operateType" :key="index" :class="currentActiveDate==type?'active':''" @click.native="operateTime(type)">{{type=='year'?t('el.datepickerGroup.year'):(type=='month'?t('el.datepickerGroup.month'):(type=='date'?t('el.datepickerGroup.day'):t('el.datepickerGroup.duration')))}}</el-button>  
    </el-button-group>
    <template v-if = "currentActiveDate == 'date'"> 
      <el-date-picker class="date" v-model = "currentDate" type="date" :placeholder="t('el.datepicker.selectDate')" :picker-options="pickerOptions" format="yyyy-MM-dd"></el-date-picker>
    </template>
    <template v-if = "currentActiveDate == 'month'"> 
      <el-date-picker class="month" v-model="currentDate" type="month" :placeholder="t('el.datepicker.selectMonth')" :picker-options="pickerMonthOptions"></el-date-picker> 
    </template>
    <template v-if = "currentActiveDate == 'year'"> 
      <el-date-picker class="year" v-model="currentDate" type="year" :placeholder="t('el.datepicker.selectYear')" :picker-options="pickerYearOptions" format="yyyy"></el-date-picker>
    </template>
    <template v-if = "currentActiveDate == 'daterange'"> 
      <el-date-picker class = "range" v-model = "currentDate" type="daterange" :placeholder="t('el.datepicker.selectDuration')"></el-date-picker>
    </template>
  </div>
</div>
</template>
