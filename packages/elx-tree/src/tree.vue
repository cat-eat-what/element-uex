...
<script>
  ...
  export default {
        ...
        return (
          <div class={nodeClass} on-dblclick={ ($event) => (this.nodeDblClick(node, data, store, $event)) } on-contextmenu={ ($event) => (this.nodeClick(node, data, store, $event)) }>
            ...
            <el-tooltip class="item" open-delay={1000} effect="light" content={data.label} placement="right">
              <span>
                {!data.icon ? (node.expanded ? (<span class="uex-icon-folder-open"></span>) : (<span class="uex-icon-folder"></span>)) : (<span class={data.icon}></span>)}
    ...
    mounted: function() {
      var _self = this;
      this.$nextTick(function() {
        _self.addEvent(document.body, 'click', function(event) {
