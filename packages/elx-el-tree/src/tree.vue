...
<script>
  ...
  import { addClass, removeClass } from 'element-uex/src/utils/dom';
  ...
  export default {
    ...
    created() {
      ...
      this.$on('tree-node-drag-over', (event, treeNode) => {
        ...
        const draggingNode = this.dropByOutside ? {} : dragState.draggingNode;
        if (!this.dropByOutside && (!draggingNode || !dropNode)) return;
        ...
        if (!this.dropByOutside && (draggingNode.node === dropNode.node || draggingNode.node.contains(dropNode.node))) {
          dropPrev = false;
          dropInner = false;
          dropNext = false;
        }
        ...
        dropIndicator.style.top = this.$el.scrollTop + indicatorTop + 'px';
        dropIndicator.style.left = this.$el.scrollLeft + (targetPosition.right - treePosition.left) + 'px';
      ...
      this.$on('tree-node-drag-leave', (event, treeNode) => {
        const node = findNearestComponent(event.target, 'ElTreeNode');
        removeClass(node.$el, 'is-drop-inner');
      });
      ...
      this.$on('tree-node-drag-end', (event) => {
        ...
        if ((this.dropByOutside || draggingNode) && dropNode) {
          if (draggingNode) {
          ...
          removeClass(dropNode.$el, 'is-drop-inner');
          const realDraggingNode = this.dropByOutside ? {} : dragState.draggingNode;

          this.$emit('node-drag-end', realDraggingNode.node, dropNode.node, dropType, event);
          if (dropType !== 'none') {
            this.$emit('node-drop', realDraggingNode.node, dropNode.node, dropType, event);
          }
        }
