<template>
  <div class="elx-cron-content" @click="focus" :style="{ minWidth: minWidth,maxWidth: maxWidth,maxHeight: maxHeight}">
    <el-tabs type="border-card" v-model="activeTabName">
      <el-tab-pane label="秒" name="secondly" v-if="visibleTypes.indexOf('secondly') > -1">
        <second ref="second" v-model="expressionModel.expressionSplit.second" @error="handleError"></second>
      </el-tab-pane>
      <el-tab-pane label="分钟" name="minutely" v-if="visibleTypes.indexOf('minutely') > -1">
        <minute ref="minute" v-model="expressionModel.expressionSplit.minute" @error="handleError"></minute>
      </el-tab-pane>
      <el-tab-pane label="小时" name="hourly" v-if="visibleTypes.indexOf('hourly') > -1">
        <hour ref="hour" v-model="expressionModel.expressionSplit.hour" @error="handleError"></hour>
      </el-tab-pane>
      <el-tab-pane label="日" name="daily" v-if="visibleTypes.indexOf('daily') > -1">
        <day ref="day" v-model="expressionModel.expressionSplit.day" @error="handleError"></day>
      </el-tab-pane>
      <el-tab-pane label="月" name="monthly" v-if="visibleTypes.indexOf('monthly') > -1">
        <month ref="month" v-model="expressionModel.expressionSplit.month" @error="handleError"></month>
      </el-tab-pane>
      <el-tab-pane label="周" name="weekly" v-if="visibleTypes.indexOf('weekly') > -1">
        <week ref="week" v-model="expressionModel.expressionSplit.week" @error="handleError"></week>
      </el-tab-pane>
      <el-tab-pane label="年" name="yearly" v-if="visibleTypes.indexOf('yearly') > -1">
        <year ref="year" v-model="expressionModel.expressionSplit.year" @error="handleError"></year>
      </el-tab-pane>
    </el-tabs>
    <div class="cron-expression">
      <el-form :model="expressionModel" label-width="90px">
        <el-form-item label="表达式字段: ">
          <div>
            <el-form class="expression" :inline="true" :model="expressionModel.expressionSplit">
              <el-form-item label="秒">
                <el-input v-model="expressionModel.expressionSplit.second" disabled></el-input>
              </el-form-item>
              <el-form-item label="分钟">
                <el-input v-model="expressionModel.expressionSplit.minute" disabled></el-input>
              </el-form-item>
              <el-form-item label="小时">
                <el-input v-model="expressionModel.expressionSplit.hour" disabled></el-input>
              </el-form-item>
              <el-form-item label="日">
                <el-input v-model="expressionModel.expressionSplit.day" disabled></el-input>
              </el-form-item>
              <el-form-item label="月">
                <el-input v-model="expressionModel.expressionSplit.month" disabled></el-input>
              </el-form-item>
              <el-form-item label="星期">
                <el-input v-model="expressionModel.expressionSplit.week" disabled></el-input>
              </el-form-item>
              <el-form-item label="年">
                <el-input v-model="expressionModel.expressionSplit.year" disabled></el-input>
              </el-form-item>
            </el-form>
          </div>
        </el-form-item>
        <!--<el-form-item label="Cron表达式: ">
          <el-input v-model="expressionModel.expression" disabled>
            <template slot="append">
              <span  @click="getExpressionSplit" style="cursor:pointer">反解析到UI</span>
            </template>
          </el-input>
        </el-form-item>-->
        <el-form-item label="*" class="error" v-if="errorMessage!=''">
          <p>{{errorMessage + ' 请重新填写！'}}</p>
        </el-form-item>
      </el-form>
    </div>
    <div class="run-time" v-if="runTimeUrl">
      <el-button class="lasted-time" @click.native="getRunTimes">获取最近5次运行时间</el-button>
      <ul>
        <li v-for="runTime in runTimes" v-text="runTime"></li>
      </ul>
    </div>
