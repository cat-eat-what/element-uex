    ...
    render(h) {
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        const btnClose = closable
          ? <span class="uex-icon-cross" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;

        const hoverLine = <svg class="hoverLine" width="100%" height="1px" version="1.1" xmlns="http://www.w3.org/2000/svg" style="position: absolute;top: 0px;left: 0px;" ><rect width="100%" height="1" style="fill:#147BD1;stroke-width: 1;stroke:#147BD1;" /></svg>;

        const tabLabelContent = pane.$slots.label || pane.label;
        const tabLabel = pane.label;
        return (
          <el-tooltip class="item" effect="dark" content={tabLabel} placement="bottom">
            <div
              class={{
                'el-tabs__item': true,
              ...
              ref="tabs"
              refInFor
              on-contextmenu={(ev) => { onTabContextmenu(pane, tabName, ev); }}
              on-click={(ev) => { onTabClick(pane, tabName, ev); }}
            >
              <span>{tabLabelContent}</span>
              {btnClose}
              {hoverLine}
            </div>
          </el-tooltip>
