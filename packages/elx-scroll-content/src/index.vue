<template>
    <div class="elx-scroll-content" :class="{'elx-row': splitType=='row', 'elx-col': splitType=='col'}" :style="{height: typeof height == 'number' ? height + 'px' : height, width: typeof width == 'number' ? width + 'px' : width, overflow: overflow}">
      <slot>无分发内容</slot>
    </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      getInnerWidth: function(el) {
        var innerWidth = 0;
        var css = this.getCss(el);
        innerWidth = this.formatVal(css.width, el.offsetWidth, -1) -
          this.parseFloat(css.paddingLeft) -
          this.parseFloat(css.paddingRight) -
          this.parseFloat(css.borderLeftWidth) -
          this.parseFloat(css.borderRightWidth);
      ...
      getOuterWidth: function(el) {
        ...
        outerWidth = this.parseFloat(css.marginLeft) +
          this.formatVal(css.width, el.offsetWidth, 1) +
          this.parseFloat(css.marginRight);
        return outerWidth;
      },
      ...
      refreshHeight: function() {
        ...
        var filterItems = _.filter(childNodes, function(node) {
