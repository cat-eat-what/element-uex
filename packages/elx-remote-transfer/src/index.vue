  ...
  <div class="elx-remote-transfer">
    <div class="elx-remote-transfer-left">
      <el-input
        ...
        <el-select v-if="searchType=='select-input'" v-model="leftSearchCol" slot="prepend" placeholder="请选择">
          <el-option v-for="col in searchCols" :key="col.value" :value="col.value" :label="col.label"></el-option>
        </el-select>
      </el-input>
      <el-table
    ...
    <div class="elx-remote-transfer-right">
      <el-input
        ...
        <el-select v-if="searchType=='select-input'" v-model="rightSearchCol" slot="prepend" placeholder="请选择">
          <el-option v-for="col in searchCols" :key="col.value" :value="col.value" :label="col.label"></el-option>
        </el-select>
      </el-input>
      <el-table
...
<script>
  ...
  export default {
    ...
    data: function() {
      var leftSearchCol = null;
      var rightSearchCol = null;
      if (this.searchType === 'select-input' && this.searchCols.length > 0) {
        leftSearchCol = this.searchCols[0].value;
        rightSearchCol = this.searchCols[0].value;
      }
      return {
        leftSearchCol: leftSearchCol,
        rightSearchCol: rightSearchCol,
        transferLeftData: [],
        transferRightData: [],
    ...
    methods: {
      getTransferLeftData: function() {
        ...
        if (self.props.search) {
          ...
          } else if (this.searchType === 'select-input') {
            filterData[self.leftSearchCol] = self.transferLeftFilter.search;
          }
        }
      ...
      getTransferRightData: function() {
        ...
        var filterData = this.currentTransferValue.filter(function(item) {
          ...
          } else if (self.searchType === 'select-input') {
            return String(item[self.rightSearchCol]).toLowerCase().indexOf(String(self.transferRightFilter.search).toLowerCase()) > -1;
          }
        });
